package com.idata.sale.service.web.base.tool.service;
...
public class HttpClientService {
    ...
    public HttpResponse doPostByForm(String url, Map<String, Object> requestParameter) throws Exception {
        HttpPost httpPost = new HttpPost(url);
        httpPost.setConfig(config);
        httpPost.addHeader(HttpHeaders.CONTENT_TYPE, ContentType.APPLICATION_FORM_URLENCODED.getMimeType());
        httpPost.addHeader(HttpHeaders.HOST, "retail.idatachina.com");

        if (requestParameter != null) {
            List<NameValuePair> form = new ArrayList<>();
            Set<Entry<String, Object>> entries = requestParameter.entrySet();
            for (Entry<String, Object> entry : entries) {
                form.add(new BasicNameValuePair(entry.getKey(),
